...
Slight changes to make it work with env setup.
"""
...
from bot_transfer.algs.her.replay_buffer import HindsightExperienceReplayWrapper
...
class SAC(OffPolicyRLModel):
    ...
    def learn(self, total_timesteps, callback=None,
        ...
        if isinstance(self.replay_buffer, HindsightExperienceReplayWrapper):
            self.add_info_in_replay_buffer = True
        else:
            self.add_info_in_replay_buffer = False
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            for step in range(total_timesteps):
                ...
                if self.num_timesteps < self.learning_starts or np.random.rand() < self.random_exploration:
                    ...
                    if isinstance(self.replay_buffer, HindsightExperienceReplayWrapper) and self.replay_buffer.relative:
                        # If were using HER and a relative structure, we need to make sure that the obs we feed to our
                        # policy are converted to relative just like those obs in the replay buffer.
                        predict_obs = obs.copy()
                        predict_obs_dict = self.env.convert_obs_to_dict(predict_obs)
                        predict_obs_dict['achieved_goal'] = predict_obs_dict['desired_goal'] - predict_obs_dict['achieved_goal']
                        predict_obs_dict['desired_goal'] = np.zeros(predict_obs_dict['desired_goal'].shape)
                        predict_obs = self.env.convert_dict_to_obs(predict_obs_dict)
                    else:
                        predict_obs = obs
                    
                    action = self.policy_tf.step(predict_obs[None], deterministic=False).flatten()
                ...
                if self.add_info_in_replay_buffer:
