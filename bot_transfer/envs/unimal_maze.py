...
class MazeEnd_Unimal(Env):
    ...
    def __init__(self, walker=None, asset_dir=None, goal_tolerance=None):
        ...
        if walker is None:
            walker = self.manifest["walkers"][0]
        if walker not in self.manifest["walkers"]:
    ...
    def reset(self):
        ...
        qpos[7:] += self.np_random.uniform(low=-0.005, high=0.005, size=len(qpos) - 7)
        qvel[6:] += self.np_random.uniform(low=-0.005, high=0.005, size=len(qvel) - 6)
